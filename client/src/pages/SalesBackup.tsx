...
import { TrendingUp, DollarSign, Package, Store, ArrowUp, ArrowDown } from 'lucide-react';
...
const TabButton = ({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) => (
  <button
    onClick={onClick}
...
const Sales = () => {
    ...
    try {
      ...
      if (tokenObj) {
        const requests = [
          ...
          axios.get(
            `${import.meta.env.VITE_SERVER_HEAD}/api/sales/all?period=${timeFrame}&page=2`,
            { withCredentials: true }
          ),
          axios.get(
            `${import.meta.env.VITE_SERVER_HEAD}/api/sales/all?period=${timeFrame}&page=3`,
            { withCredentials: true }
          ),
        ];
        ...
        const totalSales = allSales.reduce((sum, sale) => sum + sale.soldprice, 0); // Adjust key
        const totalProfit = allSales.reduce((sum, sale) => sum + sale.totalprofit, 0); // Adjust key
        const totalCommission = allSales.reduce((sum, sale) => sum + sale.commission, 0); // Adjust key
  ...
  const calculateMetrics = () => {
    ...
    const totalUnits = sales.reduce((sum, item) => sum + item.soldunits, 0);
    ...
    sales.forEach((item) => {
      ...
      shop.sales += item.soldprice * item.soldunits;
      shop.units += item.soldunits;
    ...
    sales.forEach((item) => {
      ...
      product.sales += item.soldprice * item.soldunits;
      product.units += item.soldunits;
  ...
  return (
    <div className="py-6 md:px-4 mx-auto">
      <div className="mb-8">
        <h1 className="text-title-lg font-bold text-black dark:text-white mb-2">Sales Analytics</h1>
        <div className="flex justify-between items-center">
          <p className="text-bodydark">Comprehensive sales performance insights</p>
          <select
            value={timeFrame}
            onChange={(e) => {setTimeFrame(e.target.value); setSalesData({ sales: [], totalSales: 0, totalProfit: 0 });}}
      ...
      {activeTab === 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
          <div className="bg-white dark:bg-boxdark rounded-lg shadow-card p-6 border border-stroke dark:border-strokedark">
            <h2 className="text-title-sm font-semibold mb-4 text-black dark:text-white">Revenue Trend</h2>
            <div className="h-90">
              <ResponsiveContainer>
                <LineChart data={metrics.productMetrics}>
                  <XAxis dataKey="name" stroke="#8A99AF" />
                  <YAxis stroke="#8A99AF" />
                  <Tooltip 
                    contentStyle={{ 
                  ...
                  />
                  <Line type="monotone" dataKey="sales" stroke="#42C8B7" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          <div className="bg-white dark:bg-boxdark rounded-lg shadow-card p-6 border border-stroke dark:border-strokedark">
            <h2 className="text-title-sm font-semibold mb-4 text-black dark:text-white">Revenue Distribution</h2>
            <div className="h-90">
              <ResponsiveContainer>
                <PieChart>
                  <Pie
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-stroke dark:divide-strokedark">
                {(activeTab === 1 ? metrics.productMetrics : metrics.shopMetrics).map((item, index) => (
                  <tr key={index} className="hover:bg-gray-2 dark:hover:bg-meta-4 transition-colors duration-200">
